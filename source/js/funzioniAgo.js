...
export function toCapitalize (stringa){
    ...
    if(stringa) //controllo se esiste
    {
            ...
            if(stringa.length > 0)
                {
                for(let i=0; i < stringa.length; i++) // controlliamo se la stringa è formata da più parole
                {
                    ...
                    if(caratteri_dopo_maiuscola.includes(stringa[i])) 
                    {
                        ...
                        {
                            stringa=stringa.substr(0,i+1) + stringa.charAt(i+1).toUpperCase() + stringa.slice(i+2);
